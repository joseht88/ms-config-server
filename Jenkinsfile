node {
   ...
   stage('Deployment') {
       try {
			sh 'chown jenkins:jenkins /var/lib/jenkins/workspace/ms-config-server/runDeployment.sh'
			sh 'chmod +x /var/lib/jenkins/workspace/ms-config-server/runDeployment.sh'
            sh '/var/lib/jenkins/workspace/ms-config-server/runDeployment.sh'
       }catch (e){
